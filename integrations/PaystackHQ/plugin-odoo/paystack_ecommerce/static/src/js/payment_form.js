odoo.define('paystack_ecommerce.payment_form', function (require) {
"use strict";
     ...
     payEvent: function (ev) {
        ...
        if (checked_radio.length === 1) {
            ...
            // if the user is going to pay with a form payment, then
                ...
                //console.log(checked_radio.data-provider);
                var acquirer_id = this.getAcquirerIdFromRadio(checked_radio);
                var paystack_id = parseInt(document.getElementById('paystack_id').value);
                if (acquirer_id == paystack_id) {
                    if (document.getElementById('paystack_currencies'))
                    {
                     var $allowed_currencies = document.getElementById('paystack_currencies').value;
                     var $current_currency = document.getElementById('order_currency').value;
                     if ($allowed_currencies.includes($current_currency))
                     {}
                     else{
                        alert("Paystack does not support currency: " + $current_currency);
                        return false;
                     }
                    }
                }
