<?php
    ...
    if(Params::getParam('plugin_action')=='done') {
        ...
        osc_set_preference('paystack_enabled', Params::getParam("paystack_enabled") ? Params::getParam("paystack_enabled") : '0', 'payment', 'BOOLEAN');
        osc_set_preference('paystack_sandbox', Params::getParam("paystack_sandbox") ? Params::getParam("paystack_sandbox") : '0', 'payment', 'BOOLEAN');
        osc_set_preference('paystack_tpk', Params::getParam("paystack_tpk"), 'payment', 'STRING');
        osc_set_preference('paystack_tsk', Params::getParam("paystack_tsk"), 'payment', 'STRING');
        osc_set_preference('paystack_lpk', Params::getParam("paystack_lpk"), 'payment', 'STRING');
        osc_set_preference('paystack_lsk', Params::getParam("paystack_lsk"), 'payment', 'STRING');
...
<script type="text/javascript" >
    $(document).ready(function(){
        ...
        $("#dialog-coinjar").dialog({
         ...
         $("#dialog-paystack").dialog({
            autoOpen: false,
            modal: true,
            width: '90%',
            title: '<?php echo osc_esc_js( __('Paystack help', 'payment') ); ?>'
        });
    });
</script>
        ...
        <form name="payment_form" action="<?php echo osc_admin_base_url(true); ?>" method="post">
            ...
            <fieldset>
                <div class="form-horizontal">
                    <h2 class="render-title separate-top"><?php _e('Paystack settings', 'payment'); ?> 
                        <span><a href="javascript:void(0);" onclick="$('#dialog-paystack').dialog('close');" ><?php _e('help', 'payment'); ?></a></span> 
                        <span style="font-size: 0.5em" ><a href="javascript:void(0);" onclick="$('.paystack').toggle();" ><?php _e('Show options', 'payment'); ?></a></span></h2>
                    <div class="form-row paystack ">
                        <div class="form-label"><?php _e('Enable Paystack'); ?></div>
                        <div class="form-controls">
                            <div class="form-label-checkbox">
                                <label>
                                    <input type="checkbox" <?php echo (osc_get_preference('paystack_enabled', 'payment') ? 'checked="true"' : ''); ?> name="paystack_enabled" value="1" />
                                    <?php _e('Enable Paystack as a method of payment', 'payment'); ?>
                                </label>
                            </div>
                        </div>
                    </div>
                    <div class="form-row paystack ">
                        <div class="form-label"><?php _e('Enable Test Mode'); ?></div>
                        <div class="form-controls">
                            <div class="form-label-checkbox">
                                <label>
                                    <input type="checkbox" <?php echo (osc_get_preference('paystack_sandbox', 'payment') ? 'checked="true"' : ''); ?> name="paystack_sandbox" value="1" />
                                    <?php _e('Use test API keys', 'payment'); ?>
                                </label>
                            </div>
                        </div>
                    </div>
                    <div class="form-row paystack ">
                        <div class="form-label"><?php _e('Paystack Test Secret Key', 'payment'); ?></div>
                        <div class="form-controls"><input type="text" class="xlarge" name="paystack_tsk" value="<?php echo osc_get_preference('paystack_tsk', 'payment'); ?>" /></div>
                    </div>
                    <div class="form-row paystack ">
                        <div class="form-label"><?php _e('Paystack Test Public Key', 'payment'); ?></div>
                        <div class="form-controls"><input type="text" class="xlarge" name="paystack_tpk" value="<?php echo osc_get_preference('paystack_tpk', 'payment'); ?>" /></div>
                    </div>
                    <div class="form-row paystack ">
                        <div class="form-label"><?php _e('Paystack Live Secret Key', 'payment'); ?></div>
                        <div class="form-controls"><input type="text" class="xlarge" name="paystack_lsk" value="<?php echo osc_get_preference('paystack_lsk', 'payment'); ?>" /></div>
                    </div>
                    <div class="form-row paystack ">
                        <div class="form-label"><?php _e('Paystack Live Public Key', 'payment'); ?></div>
                        <div class="form-controls"><input type="text" class="xlarge" name="paystack_lpk" value="<?php echo osc_get_preference('paystack_lpk', 'payment'); ?>" /></div>
                    </div>
...
<form id="dialog-paystack" method="get" action="#" class="has-form-actions hide">
    <div class="form-horizontal">
        <div class="form-row">
            <h3><?php _e('Learn more about Paystack', 'payment'); ?></h3>
            <p>
                <?php printf(__('Stripe official website: %s', 'payment'), '<a href="https://stripe.com/">https://stripe.com/</a>'); ?>.
                <br/>
                <?php printf(__('Getting started: %s', 'payment'), '<a href="https://stripe.com/docs">https://stripe.com/docs</a>'); ?>.
        ...
        <div class="form-actions">
            <div class="wrapper">
                <a class="btn" href="javascript:void(0);" onclick="$('#dialog-paystack').dialog('close');"><?php _e('Cancel'); ?></a>
            </div>
        </div>
    </div>
</form>